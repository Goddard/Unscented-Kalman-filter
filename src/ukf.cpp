...
UKF::UKF() {
  ...
  */
  ...
  // initial state vector of size n_x_
  x_ = VectorXd(n_x_);

  // initial covariance matrix
  //matrix size
  P_ = MatrixXd(n_x_, n_x_);
  //fill with zero point zero
  P_.fill(0.0);
  //one point zero diagonally
...
void UKF::ProcessMeasurement(MeasurementPackage meas_package) {
  ...
  *  Prediction
  ****************************************************************************/
  double delta_t = (meas_package.timestamp_ - time_us_) / 1000000.0;    //dt
...
//  P_ = P_ - K*S*K.transpose();
  ...
  Also calculate the lidar NIS (Normalized innovation squared).
  */
...
//  P_ = P_ - K*S*K.transpose();
 ...
 position. Modify the state vector, x_, and covariance, P_.
 Calculate the radar NIS (normalized innovation squared).
 */
